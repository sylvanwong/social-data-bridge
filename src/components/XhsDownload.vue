...
const shouldProxyAttachmentDownload = (item, fieldName) => {
  return fieldName === '下载附件' && item?.note_type === '视频';
};

const downloadAttachmentAsFile = async (url, finalName, fieldName, item) => {
...
const TASK_PLUGIN_TYPE = 'xhs_download';
...
const loadTargetFieldOptions = async (tableId) => {
  ...
  try { const table = await bitable.base.getTableById(tableId); targetFieldOptions.value = await table.getFieldMetaList(); } catch (error) { targetFieldOptions.value = []; }
};
...
const resolveFieldId = (config, fieldNameToId) => mappingDraft.value.find(item => item?.source_key === config.key && item?.target_field_id)?.target_field_id || fieldNameToId[config.name];
...
const getAllVisibleRecordIdList = async (view, limit = Infinity) => {
  const pageSize = 200;
  ...
  while (loopCount < maxLoops) {
    ...
    if (recordIds.length > 0) {
      ...
      if (allRecordIdList.length >= limit) {
        return allRecordIdList.slice(0, limit);
      }
    }
...
const getRecordIdListByScope = async (scope, rowCount) => {
  ...
  if (scope === 'all') {
    ...
    recordIdList = await getAllVisibleRecordIdList(view, rowCount);
  }
  return recordIdList;
};
...
const validateAndAddFields = async (tableId, activeFieldConfigs) => {
  try {
    ...
    for (const config of activeFieldConfigs) {
      const mappedFieldId = mappingDraft.value.find(mapping => mapping?.source_key === config.key && mapping?.target_field_id)?.target_field_id;
      if (mappedFieldId) {
        const mappedField = fieldList.find(field => field.id === mappedFieldId);
        if (!mappedField) {
          ElNotification({ message: `映射目标字段不存在: ${config.name}`, type: 'error', duration: 0 });
          return null;
        }
        if (!isFieldTypeCompatible(mappedField.type, config)) {
          typeMismatchFields.push({ name: `${config.name} → ${await mappedField.getName()}`, expected: getAllowedFieldTypes(config).map(type => FIELD_TYPE_NAME[type] || type).join(' / '), actual: mappedField.type });
        }
        continue;
      }
...
const getFinalFileName = (url, baseName, index, total) => {
  ...
  const ext = extMatch ? `.${extMatch[1]}` : (url.includes('.mp4') ? '.mp4' : '.jpg');
...
const writeDataToRecord = async (recordId, item, fieldNameToId, activeFieldConfigs) => {
  try {
    ...
    for (const config of activeFieldConfigs) {
      const fieldId = resolveFieldId(config, fieldNameToId);
      if (!fieldId) continue;
      ...
      try {
        ...
        if (config.type === FieldType.Attachment) {
          ...
          // 过滤掉空值，自动把 HTTP 转 HTTPS 解决混合内容问题
          urls = urls
...
const writeDataToTargetRecord = async (table, item, activeFieldConfigs, fieldIdByName) => {
  const cells = [];

  for (const config of activeFieldConfigs) {
    const fieldId = resolveFieldId(config, Object.fromEntries(fieldIdByName));
    if (!fieldId) continue;
    ...
    if (config.type === FieldType.Attachment) {
      ...
      if (urls.length > 0) {
        try {
          ...
          cells.push(await field.createCell(files.length === 1 ? files[0] : files));
        } catch (error) {
          console.log('附件下载失败，跳过附件写入:', error);
          cells.push(await field.createCell(null));
        }
      } else {
        cells.push(await field.createCell(null));
      }
      ...
      const value = config.name === '平台' && fieldType === FieldType.SingleSelect
        ? (config.getValue(item) || null)
        : config.getValue(item);
      cells.push(await field.createCell(value));
    }
  }
...
const appendRecordsToTable = async (tableId, list, activeFieldConfigs) => {
  ...
  for (const item of list) {
    const cells = await writeDataToTargetRecord(table, item, activeFieldConfigs, fieldIdByName);
    if (cells.length > 0) {
      records.push(cells);
    }
  }
  ...
  if (Array.isArray(value)) {
    for (const item of value) {
      if (item.type === 'url' && item.link) {
        ...
        if (link.includes('xhslink.com') || link.includes('xiaohongshu.com')) {
          return link;
        }
      }
    }
  ...
  const rows = await Promise.all(
    recordIdList.map(async (recordId) => {
      try {
        ...
        const url = extractXhsLink(value);
...
const handleTableModeSubmit = async () => {
  ...
  if (!formData.value.xhsLinkFieldId) {
    ElNotification({ message: '请选择小红书链接字段', type: 'warning', duration: 0 });
    return;
  }

  if (formData.value.xhsLinkFieldId === 'nodata') {
...
const handleManualModeSubmit = async () => {
  ...
  if (!formData.value.manualUrls || !formData.value.manualUrls.trim()) {
    ElNotification({ message: '请输入小红书链接', type: 'warning', duration: 0 });
    return;
  }
  ...
  if (urls.length === 0) {
    ElNotification({ message: '请至少输入一个有效的小红书链接', type: 'warning', duration: 0 });
    return;
  }
...
<template>
  <div class="sub-page">
    ...
    <div class="form-card">
      ...
      <div class="group-label">小红书链接来源</div>
          ...
          <el-form-item label="">
            <div slot="label" class="c-label">
              ...
              <el-tooltip effect="dark" placement="top">
                <template #content>支持小红书作品链接</template>
                <img src="https://cdn.zhinizhushou.com/material/20250826/45c287c837d7c34626a8f441264db162.png"
                  class="help-icon" />
              </el-tooltip>
            </div>
            <el-select
              v-model="formData.xhsLinkFieldId"
              placeholder="选择包含小红书链接的字段"
              style="width: 100%"
            >
...
.mapping-accordion-label { display:flex; align-items:baseline; gap:4px; font-size:14px; font-weight:500; }
